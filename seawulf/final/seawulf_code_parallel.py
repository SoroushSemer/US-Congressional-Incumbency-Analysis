...
CORES = 4   
# STATE = sys.argv[2]
# REP_THRESHOLD = int(sys.argv[3])
# DEM_THRESHOLD = int(sys.argv[4])
# POP_THRESHOLD = float(sys.argv[5])
# GEO_THRESHOLD = float(sys.argv[6])
...
def export_plan(initial_partition, partition, precincts, description):
    ...
    if(os.path.isfile("./"+STATE+"_GeneratedPlan_"+description+".json")): return
    ...
    colors = ['red', 'blue', 'green', 'yellow', 'cyan', 'magenta', 'purple', 'orange',  'black', 'brown', 'pink', 'gray', 'olive', 'teal', 'navy', 'gold', 'silver', 'tan', 'lime', 'aqua', 'maroon', 'lavender', 'beige', 'chocolate', 'indigo', 'violet']
    ...
    try:
        district.to_file("./"+STATE+"_GeneratedPlan_"+description+".json")
...
def gen_plan(seed):
    ...
    chain = MarkovChain(
            proposal=proposal,
            constraints=[no_vanishing_districts,  compactness_bound],
    ...
    for partition in chain.with_progress_bar():
        ...
        # partition.plot()
        if(partition['election'].seats('Republican')>REP_THRESHOLD):
            export_plan(initial_partition, partition, precincts, "REP_Favored ("+str(partition['election'].seats('Republican'))+" wins)")       
    
        elif(partition['election'].seats('Democratic')>DEM_THRESHOLD):
            export_plan(initial_partition, partition, precincts, "DEM_Favored ("+str(partition['election'].seats('Democratic'))+" wins)")
    
        if(max(var['popVar']) > POP_THRESHOLD):
            export_plan(initial_partition, partition, precincts, "High_Pop_Var ("+str(round(max(var['popVar']),3))+" max)")
    
        if(max(var['areaVar']) > GEO_THRESHOLD):
            export_plan(initial_partition, partition, precincts, "High_Geo_Var ("+str(round(max(var['areaVar']),3))+" max)")
...
if __name__ == '__main__':
    ...
    print()
    print(STATE,"2020 Graph Created")
